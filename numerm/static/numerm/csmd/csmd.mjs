...
export class Digraph extends D3Base {
	...
	render() {
		...
		this.FORCE.on("tick", function () {
			edges.attr("d", (d) => {
				if (d.curve && d.curve === "up") {
					const midY = (d.source.x + d.target.x) / 2;
					return `M${d.source.x},${d.source.y} S${midY},${4} ${
						d.target.x
					}, ${d.target.y}`;
				}
				if (d.curve && d.curve === "down") {
					const midY = (d.source.x + d.target.x) / 2;
					return `M${d.source.x},${d.source.y} S${midY},${midY} ${d.target.x}, ${d.target.y}`;
				}
...
export class Tree {
	...
	render() {
		...
		const dataField = labels
			...
			.attr("text-anchor", "middle")
			.attr("fill", (d) =>
				d.children ? this.colors.textColor : this.colors.leafTextColor
			)
			.style("font-family", "system-ui")
...
export class CallTrace extends D3Base {
	constructor(obj) {
		...
		this.svg_width = this.OBJ.svg_width ? this.OBJ.svg_width : 430;
		this.svg_height = this.OBJ.svg_height ? this.OBJ.svg_height : 50;
		this.D3_CONTAINER_WIDTH = this.OBJ.container_width
			? `${this.OBJ.container_width}%`
			: this.containerWidthDefault;

		this.D3_CONTAINER_HEIGHT = this.OBJ.container_height
			? `${this.OBJ.container_height}%`
			: this.containerHeightDefault;
		this.COLORS = {
		...
		this.SVG_DEFS = this.SVG.append("svg:defs")
			.selectAll("marker")
			.data(["end"])
			.enter()
			.append("svg:marker")
	...
	render() {
		...
		node
			...
			.attr("r", 5)
			.attr("fill", (d) => {
				return d.children ? this.COLORS.nodeColor : this.COLORS.leafColor;
			})
			.attr("stroke", this.COLORS.strokeColor);
		...
		node
			...
			.attr("fill", (d) => {
				return d.children || d.data.call
					? this.COLORS.branchText
					: this.COLORS.leafText;
			})
			.attr("x", function (d) {
